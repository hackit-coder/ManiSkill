...
class Args:
    ...
    pc_and_goal_only: bool = False
    sampling_policy: str = 'random'
    sampling_num_samples: int = 1024
...
def get_state_and_pc(obs, env_obs_space, envs_origin_state_dim, pc_and_goal_only):
...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id}__{args.exp_name}__{args.seed}__{int(time.time())}_{args.pc_and_goal_only}_{args.sampling_policy}_{args.sampling_num_samples}_{args.num_envs}"
    writer = None
    ...
    # env setup
    env_kwargs = dict(obs_mode="pointcloud", control_mode="pd_joint_delta_pos", render_mode="rgb_array", sim_backend="gpu", sampling_config=dict(policy=args.sampling_policy,
    ...
    env_obs_space = envs.single_observation_space
    print("in original state observation space, we have keys: ", env_obs_space.keys())
    if not args.pc_and_goal_only:
        for key, value in env_obs_space['agent'].items():
            print(f"key {key} added into state observation space")
            envs_origin_state_dim += value.shape[0]
            envs_origin_state_keys.append(key)
        for key, value in env_obs_space['extra'].items():
            print(f"key {key} added into state observation space")
            if len(value.shape) < 1:
                envs_origin_state_dim += 1
            else:
                envs_origin_state_dim += value.shape[0]
            envs_origin_state_keys.append(key)
    else:
    ...
    envs_original_pointcloud_shape = (args.sampling_num_samples, 3)
    ...
    next_pc, next_state = get_state_and_pc(next_obs, env_obs_space, envs_origin_state_dim, pc_and_goal_only=args.pc_and_goal_only)
    # next_obs = next_obs['pointcloud']['xyzw'][..., :3]
    eval_obs, _ = eval_envs.reset(seed=args.seed)
    eval_pc, eval_state = get_state_and_pc(eval_obs, env_obs_space, envs_origin_state_dim, pc_and_goal_only=args.pc_and_goal_only)
